...
> 目录细节放 `core/AGENT.md`、`server/AGENT.md`，当前经验放 `lesson_learned.md`。
> 架构决策放 `docs/ADR/`，`.cursorrules` 只留 Cursor 专属极简提醒。
> 任何工具私有文件都不应成为唯一真相源。

这份文档定义 OrgMem 里与 AI 协作相关的几类文档该怎么分工、怎么控制体积、什么内容该写到哪里，避免 `CLAUDE.md`、`.cursorrules`、`AGENT.md`、`lesson_learned.md`、ADR 彼此重叠、持续长胖、最后没人敢改。

跨项目复用模板见 `docs/templates/README.md`。

## 目标

- 让 agent 在 30 秒内知道“先看哪份文档”
- 让高频规则放在最短的地方，低频细节放到更深一层
- 减少重复维护，避免同一条规则同时出现在 3 份文件里
...
- 目录级局部 runbook：`core/AGENT.md`、`server/AGENT.md`
...
当前仓库已经有目录级这层：`core/AGENT.md`、`server/AGENT.md`。继续保持“按需引入”，不要为了对称性再建一个重复全仓库规则的根 `AGENT.md` 空壳。

什么时候该加：

- `core/` 和 `server/` 开始拥有明显不同的运行方式、测试方式、风险点
...
- 索引、搜索、chunk、embedding、runtime memory

## 进入前先知道

- 不要改写 review 流程
- 不要破坏 delete-before-insert + UUID5
...
基于 OrgMem 现在的状态，我建议这样收敛：

- `CLAUDE.md`：继续保持“短主文件”，只放高频全局规则
- `.cursorrules`：继续压到 3-5 条，只保留语言和极少数高价值纠错记忆
- 根级 `AGENTS.md`：允许存在，但只保留 3-6 行跨工具指针
- `AGENT.md`：保留现有目录级 `core/AGENT.md`、`server/AGENT.md`；暂时不要在仓库根目录再建重复版
- `lesson_learned.md`：继续只保留当前有效经验，不回填 ADR 摘要
